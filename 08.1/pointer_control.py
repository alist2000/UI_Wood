...
def pointer_control(range_x, range_y, x, y):
    if (x in range(int(range_x[0]), int(range_x[1]) + 1)) and (y in range(int(range_y[0]), int(range_y[1]) + 1)):
        return True
...
def pointer_control_shearWall(x, y, x_grid, y_grid):
    x_start = x_grid[0]
    x_end = x_grid[-1]
    y_start = y_grid[0]
    y_end = y_grid[-1]
    # point is on horizontal grid
    if (x in [i for i in np.arange(x_start, x_end, 0.5)]) and (y in [i for i in y_grid]):
        if y == y_grid[0] or y == y_grid[-1]:
            int_ext = "exterior"
        else:
            int_ext = "interior"
        return True, "E-W", int_ext

    # point is on vertical grid
    elif (y in [i for i in np.arange(y_start, y_end, 0.5)]) and (x in [i for i in x_grid]):
        if x == x_grid[0] or x == x_grid[-1]:
            int_ext = "exterior"
        else:
            int_ext = "interior"
        return True, "N-S", int_ext
    else:
        return False, "", ""
