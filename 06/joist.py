...
class JoistArea:
    def __init__(self, x, y, spacing_x, spacing_y, scene, joist_select):
        # note spacing x = width, spacing y = height
        show_values = [
            (x[i] + spacing_x[i] / 10, y[j] + spacing_y[j] / 10,
             (spacing_x[i] - spacing_x[i] / 5),
             (spacing_y[j] - spacing_y[j] / 5))
            for i, j in itertools.product(range(len(spacing_x)), range(len(spacing_y)))]
        values = [
            (x[i], y[j],
             (spacing_x[i]),
             (spacing_y[j]))
...
class ClickableRectItem(QGraphicsRectItem):
    def __init__(self, x, y, width, height, associated_rect, joist, Joist_Position, image):
        ...
        self.joist_select = 0  # 0: neutral, 1: select joist, 2: delete joist
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.RightButton:
            ...
            if self.joist_select:
                visibility = True if self.joist_select == 1 else False
                self.associated_rect.setVisible(visibility)
                if self.joist_select == 1:
